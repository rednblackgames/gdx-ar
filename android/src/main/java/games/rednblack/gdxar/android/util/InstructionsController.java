...
import games.rednblack.gdxar.R;
...
public class InstructionsController {
    ...
    public InstructionsController(Activity activity, LayoutInflater inflater, FrameLayout container) {
    ...
    protected View onCreateView(int type) {
        ...
        switch (type) {
            case TYPE_PLANE_DISCOVERY:
                view = inflater.inflate(R.layout.instructions_plane_discovery, container, false);
                break;
            case TYPE_AUGMENTED_IMAGE_SCAN:
                view = inflater.inflate(R.layout.instructions_augmented_image, container, false);
                break;
        }
        if (view != null) {
    ...
    public boolean isEnabled(int type) {
        return Boolean.TRUE.equals(typesEnabled.getOrDefault(type, true));
    }
    ...
    public boolean isVisible(int type) {
        return Boolean.TRUE.equals(typesVisibilities.getOrDefault(type, false));
    }
    ...
    private void updateVisibility() {
        activity.runOnUiThread(() -> {
            for (int type : typesVisibilities.keySet()) {
