...
/**
 * * * NOTE: This is a modified file from the original ObjLoader. @claywilkinson added a code to
 * allow multiple groups each with their own material.
 */

/**
 * {@link ModelLoader} to load Wavefront OBJ files. Only intended for testing basic models/meshes
 * and educational usage. The Wavefront specification is NOT fully implemented, only a subset of the
 * specification is supported. Especially the {@link Material} ({@link Attributes}), e.g. the color
 * or texture applied, might not or not correctly be loaded. This {@link ModelLoader} can be used to
 * load very basic models without having to convert them to a more suitable format. Therefore it can
 * be used for educational purposes and to quickly test a basic model, but should not be used in
 * production. Instead use {@link G3dModelLoader}. Because of above reasons, when an OBJ file is
 * loaded using this loader, it will log and error. To prevent this error from being logged, set the
 * {@link #logWarning} flag to false. However, it is advised not to do so. An OBJ file only contains
 * the mesh (shape). It may link to a separate MTL file, which is used to describe one or more
 * materials. In that case the MTL filename (might be case-sensitive) is expected to be located
 * relative to the OBJ file. The MTL file might reference one or more texture files, in which case
 * those filename(s) are expected to be located relative to the MTL file.
 *
 * @author mzechner, espitz, xoppa
 */
public class ObjLoaderImproved extends ModelLoader<ObjLoaderImproved.ObjLoaderParameters> {
    /**
     * Set to false to prevent a warning from being logged when this class is used. Do not change this
     * value, unless you are absolutely sure what you are doing. Consult the documentation for more
     * information.
     */
    ...
    public ObjLoaderImproved() {
        this(null);
    }

    public ObjLoaderImproved(FileHandleResolver resolver) {
        super(resolver);
    }
    ...
    protected ModelData loadModelData(FileHandle file, boolean flipV) {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                if (tokens[0].length() == 0) {
                    ...
                    if (tokens.length == 1) activeGroup.materialName = "default";
                        ...
                        if (!activeGroup.materialName.equals("default") || activeGroup.faces.size != 0) {
                            // this is a change of material in the group, so make a sub-group
                            activeGroup = setActiveGroup(activeGroup.name + "_" + activeGroup.faces.size);
                        }
        ...
        for (int g = 0; g < numGroups; g++) {
            ...
            final float[] finalVerts =
                    new float[(numFaces * 3) * (3 + (hasNorms ? 3 : 0) + (hasUVs ? 2 : 0))];
...
class MtlLoader {
    ...
    public void load(FileHandle file) {
        ...
        ModelMaterial.MaterialType type = ModelMaterial.MaterialType.Lambert;
        float opacity = 1.f;
        float shininess = 0.f;
            ...
            while ((line = reader.readLine()) != null) {
                ...
                if (tokens[0].length() == 0) {
                    ...
                    if (key.equals("newmtl")) {
                        ...
                        type = ModelMaterial.MaterialType.Lambert;
                        opacity = 1.f;
                        shininess = 0.f;
                    } else if (key.equals("kd")
                            || key.equals("ks")
                            || key.equals("ka")) // diffuse or specular
                    {
                        ...
                        if (!ambcolor.equals(Color.WHITE) && !difcolor.equals(Color.WHITE)) {
                            ambcolor.mul(difcolor);
                        }

                    } else if (key.equals("tr") || key.equals("d")) {
                        opacity = Float.parseFloat(tokens[1]);
                    } else if (key.equals("ns")) {
                        type = ModelMaterial.MaterialType.Phong;
                        shininess = Float.parseFloat(tokens[1]);
                    } else if (key.equals("map_kd")) {
                        texFilename = file.parent().child(tokens[1]).path();
                    } else if (key.equals("illum")) {
                        Gdx.app.log("ObjLoader", " illum model is unsupported: " + line);
                    } else if (key.equals("ni")) {
                        Gdx.app.log("ObjLoader", "Optical density is unsupported: " + line);
                    } else {
                        Gdx.app.log("ObjLoader", " Unknown line in MTL file: " + line);
                    }
                }
            }
            reader.close();
